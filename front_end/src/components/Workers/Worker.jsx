...
function Worker({ id, siteId, name, image, mobile, isSettled, isSiteCompleted }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await deleteWorkerFromDB(id, siteId);
      ...
      // Backend confirmed → start fade animation
      setIsAnimatingOut(true);
      setShowActions(false); // Close dropdown menu

      // Small delay for fade-out animation
      await new Promise(resolve => setTimeout(resolve, 300));

      // Remove worker from Redux store after animation
      setTimeout(() => {
  ...
  return (
    <div
      className={`rounded-lg border group relative transition-all duration-500 ${isAnimatingOut
        ? 'opacity-0 scale-95 transform translate-y-4'
        : 'opacity-100 scale-100 transform translate-y-0'
        } ${isDeleting ? 'pointer-events-none' : ''} ${isSettled
    ...
    >
          ...
          <>
            <div className="absolute top-2 right-2 z-10">
              <button
                ...
                className={`p-1 rounded-md transition-all duration-200 ${isDeleting || isSettled
      ...
      <div className="p-4 text-center relative">
        ...
        <div className="flex justify-center mb-3">
          {image ? (
            <>
              ...
              />
              <div className={`w-16 h-16 rounded-full flex items-center justify-center hidden ${isSettled
            ...
            <div className={`w-16 h-16 rounded-full flex items-center justify-center ${isSettled
              ? 'bg-gradient-to-br from-gray-500 to-gray-600'
              : 'bg-gradient-to-br from-blue-500 to-purple-600'
              }`}>
              <User className="w-6 h-6 text-white" />
        ...
        {/* Worker Name */}
        <h3
          className={`text-sm font-semibold mb-2 truncate ${isSettled ? 'text-gray-700' : 'text-gray-900'
            }`}
        >
          {name || 'Unnamed Worker'}
        </h3>
