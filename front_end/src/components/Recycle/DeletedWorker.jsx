...
import { deleteWorkerPer, restoreWorker } from '../../services/recycleService';
import { toast } from 'react-toastify';

const API_URL = "http://localhost:3000";

function DeletedWorker({ id, name, mobile, image, deletedAt, onStateChange }) {
  ...
  const handleRestore = async () => {
    try {
      const confirm = window.confirm("Are you sure you want to restore this worker?");
      ...
      if (confirm) {
        ...
        const restoredWorker = await restoreWorker(id)
  ...
  const handlePermanentDelete = async () => {
    try {
      const confirm = window.confirm("Are you sure you want to delete this worker permanently? This action cannot be undone.");
      ...
      if (confirm) {
        ...
        const deletedWorker = await deleteWorkerPer(id)
      ...
      {isRestoring && (
        <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-20 flex items-center justify-center">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-sm font-medium text-gray-700">Restoring worker...</span>
          </div>
        </div>
      )}
